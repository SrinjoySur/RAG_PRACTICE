...
class DATABASEINJECTOR:
    ...
    def check_connection(self):
        if not self.conn:
            ...
            logger.info("[OK] Databse connection successfully established")
            with self.conn.cursor() as cur:
                cur.execute("SELECT * FROM DEMO;")
    def close_connection(self):
        if not self.conn:
            logger.error("[ERROR] Database connection not established or closed prematurely.")
        else:
            self.conn.close()
            logger.info("[OK] Databse connection closed successfully.")
